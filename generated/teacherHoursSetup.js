(function() {
	...
	let handler = createMessageHandler("HoursSettings");
	...
	function fillSubjectsTable(dko3Setup) {
		let tbody = document.getElementById("subjectsContainer").querySelector("table>tbody");
		tbody.innerHTML = "";
	...
	function addTranslationRow(trns, tbody) {
		...
		function buildField(label, value, id) {
			return `(td>{${label}})+(td>input-with-spaces#${id}[type="text"${value ? ` value="${value}"` : ""}])`;
		}
	}
	function fillTranslationsTable(cloudData) {
		let tbody = document.getElementById("translationsContainer").querySelector("table>tbody");
		tbody.innerHTML = "";
	...
	function fillGradeYearsTable(cloudData) {
		let tbody = document.getElementById("gradeYearsContainer").querySelector("table>tbody");
		tbody.innerHTML = "";
	...
	async function onData(request) {
		...
		document.getElementById("btnNewTranslationRow").addEventListener("click", function(_) {
			addTranslationRow({
				...
				prefix: "",
				suffix: ""
			}, document.querySelector("#translationsContainer tbody"));
			hasTableChanged = true;
		});
	}
	...
	function scrapeSubjects() {
		return [...document.querySelectorAll("#subjectsContainer>table>tbody>tr")].map((row) => {
			return {
	...
	function scrapeGradeYears() {
		return [...document.querySelectorAll("#gradeYearsContainer>table>tbody>tr")].map((row) => {
			return {
				checked: row.cells[0].querySelector("input:checked") !== null,
	...
	function scrapeTranslations() {
		return [...document.querySelectorAll("#translationsContainer>table>tbody>tr")].map((row) => {
			return {
	...
	function onCheckTableChanged(dko3Setup) {
		...
		saveHourSettings(setupData).then((_) => {
			sendRequest("open_hours_settings_changed", "HoursSettings", "Main", void 0, setupData).then((_) => {});
		});
	}
		...
		document.querySelectorAll(".tabs > button.tab").forEach((btn) => btn.addEventListener("click", (ev) => {
			switch (ev.target.id) {
				...
				case "btnTabTranslations":
					tabs.switch(ev.target);
					break;
				case "btnTabGradeYears": tabs.switch(ev.target);
			}
		}));
		await sendDataRequest("HoursSettings", "HoursSettings", { schoolYear: new URLSearchParams(document.location.search).get("schoolyear") });
	}
	//#endregion
})();
