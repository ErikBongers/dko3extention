(function() {
				...
				return async function onMessage(request, _sender, _sendResponse) {
					...
					if (request.targetTabType === tabType) {
						...
						if (request.targetTabId === tab.id) {
							if (request.action === "tab_data" && self._onData) self._onData(request);
	...
	function loadWebComponent() {
		class InputWithSpaces extends HTMLElement {
			...
			onInput() {
				...
				let stringArray2 = this.input.value.split(/(\s+)/).filter((slice) => slice);
	...
	async function uploadJson(fileName, data) {
		return await (await fetch("https://europe-west1-ebo-tain.cloudfunctions.net/json?fileName=" + fileName, {
			method: "POST",
	...
	let handler = createMessageHandler("diffSettings");
	...
	function addTranslationRow(tagDef, tbody) {
		...
		addDeleteButton(tr);
		tr.querySelector("#trnsIsClassName").addEventListener("change", (_) => {
	...
	function buildLabeledDecoratedTextField(label, value, id) {
		return `(td.label>{${label}})+(td>input-with-spaces#${id}[type="text"${value ? ` value="${value}"` : ""}])`;
	}
	...
	function addIgnoresRow(ignore, tbody) {
		...
		function buildField(value, id) {
			return `(td>input-with-spaces#${id}[type="text"${value ? ` value="${value}"` : ""}])`;
		}
	}
	...
	function fillIgnoresTable(diffSettings) {
		let tbody = document.getElementById("ignoresContainer").querySelector("table>tbody");
		tbody.innerHTML = "";
	...
	async function onData(request) {
		...
		document.querySelector("button").addEventListener("click", async () => {
			await sendRequest("greetingsFromChild", "Undefined", "Main", void 0, "Hullo! Fly safe!");
		});
		...
		document.getElementById("btnNewTranslationRow").addEventListener("click", function(_) {
			addTranslationRow({
				...
				searchString: "",
				gradeYears: ""
			}, document.querySelector("#tagDefsContainer tbody"));
			dataUploader.setDataChanged();
		});
		document.getElementById("btnNewPreTranslationRow").addEventListener("click", function(_) {
			addPreTranslationRow({
				trigger: "",
				search: "",
				replace: "",
				dscr: ""
			}, document.querySelector("#tagPreTransContainer tbody"));
			dataUploader.setDataChanged();
		});
		document.getElementById("btnNewIgnoresRow").addEventListener("click", function(_) {
			addIgnoresRow("", document.querySelector("#ignoresContainer tbody"));
			dataUploader.setDataChanged();
		});
		let txtUrls = document.getElementById("txtWebPages");
	...
	function scrapeTagDefs() {
		return [...document.querySelectorAll("#tagDefsContainer>table>tbody>tr")].map((row) => {
	...
	function scrapeIgnores() {
		return [...document.querySelectorAll("#ignoresContainer>table>tbody>tr")].map((row) => row.querySelector("#trnsIgnore").value);
	}
	function scrapePreTranslations() {
		return [...document.querySelectorAll("#tagPreTransContainer>table>tbody>tr")].map((row) => {
			return {
				trigger: row.querySelector("#trnsTrigger").value.trim(),
				search: row.querySelector("#trnsSearch").value.trim(),
				replace: row.querySelector("#trnsReplace").value.trim(),
				dscr: row.querySelector("#trnsDscr").value.trim()
			};
		});
	}
	async function saveIfDataChanged() {
		if (!globalSetup) return;
		let urls = document.getElementById("txtWebPages").value.split("\n").map((s) => s.trim()).filter((s) => s.length > 0);
		let setupData = {
			version: 1,
		...
		await sendRequest("diff_settings_changed", "diffSettings", "Main", void 0, setupData).then((_) => {});
	}
	async function onDocumentLoaded(_) {
		new Tabs(document.querySelector("div.tabsContainer"), [
			{
				btnId: "btnTabTagDefs",
				tabId: "tabTagDefs",
				btnContent: "Tags"
		...
		await sendDataRequest("diffSettings", "DiffSettings", {
			academie: params.get("academie"),
			schoolYear
		});
	}
	//#endregion
