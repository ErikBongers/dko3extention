(function(default_items) {
	...
	var RosterFactory = class RosterFactory {
		constructor(jsonExcelData) {
			this.errors = [];
			this.daysRow = void 0;
			this.periodColumn = void 0;
			this.tableRange = void 0;
	...
	var ExcelRoster = class {
		...
		scrapeColumn(column, timeSlices) {
			...
			for (let row = 0; row < this.table.RowCount; row++) {
				...
				if (cellValue) {
					let parseText = " " + cellValue.replaceAll(/\n/g, " ").replaceAll("(", " ( ").replaceAll(")", " ) ").replaceAll(",", " , ").replaceAll("+", " + ").replaceAll(" en ", " , ").replaceAll("  ", " ").replaceAll("  ", " ") + " ";
					...
					let classDef = new ClassDef(day, teacher, timeSlice, subjects, location, gradeYears, excelPos.row, excelPos.column, cellValue, this.table, className);
	...
	function scrapeLesInfo(row) {
		...
		if (Array.from(allBadges).some((el) => el.textContent === "module")) if (les.naam.includes("jaar")) les.lesType = 1;
		else if (les.naam.includes("rimester")) les.lesType = 0;
		else les.lesType = 3;
		else les.lesType = 2;
		...
		les.gradeYears = textsToYearGrades([...lesCell.querySelectorAll("span.text-info")].map((e) => e.textContent));
		splitLesMoment(les);
		return htmlLes;
	}
	...
	function addJaarStudentsToMapAndCount(students, jaarModule) {
		...
		for (let student of jaarModule.students) {
			...
			students.get(student.name).jaarInstruments.push(jaarModule);
		}
	...
	*/
	...
	var FetchedTable = class {
		constructor(tableDef) {
			...
			this.getNextOffset = () => this.getNextPageNumber() * this.tableFetcher.tableRef.navigationData.step;
	...
	function isColumnProbablyDate(table, index) {
		return stringToDate(Array.from(table.tBodies[0].rows)[0].cells[index].textContent);
	}
	...
	async function getAndShowDiffs(showOrCalc, useDkoCache) {
		...
		let fetchListener = new InfoBarTableFetchListener(createInfoBlock(divInfo, ""));
		let errors = [];
	...
	async function showDiffs(diffs, academie, schoolYear, dko3DiffData, diffSettings) {
		...
		let chkHideChecked = emmet.appendChild(divResults, `div#divHideChecked>(input#chkHideChecked[type="checkbox"]+label[for="chkHideChecked"]{Verberg aangevinkte lijnen})`).first.querySelector("#chkHideChecked");
		...
		chkHideChecked.checked = (localStorage.getItem("dko3plugin.hideIgnoredDiffs") ?? "false") == "true";
	...
	async function saveIgnoredHashes(academie, schoolYear) {
		await uploadIgnoredDiffHashes(academie, schoolYear, [...document.getElementById("orphans").querySelectorAll("tr.ignore")].map((tr) => tr.dataset.hash));
	}
	...
	async function buildAndSaveDiff(reportStatus, fetchListener, academie, schoolYear, dko3DiffData, diffSettings) {
		...
		for (let file of folderPath.files) {
			reportStatus(`Inlezen van ${file.name.replaceAll("Dko3/Uurroosters/", "")}...`);
	...
	async function runRosterCheck(excelDatas, reportStatus, fetchListener, dko3DiffData, diffSettings) {
		...
		for (let excelData of excelDatas) {
			let roster = new ExcelRoster(new RosterFactory(excelData).getTable(), dko3DiffData.locations, dko3DiffData.subjects, diffSettings, dko3DiffData.classNames);
	...
	var Weight = class {
		constructor() {
			this.weight = 1e3;
			this.diffTeacher = false;
			this.diffSubject = false;
			this.diffLocation = false;
			this.diffDayTime = false;
			this.diffGradeYears = 0;
		}
		calcWeight() {
			this.weight = 1e3;
	...
	var ExtraTeacherCache = class ExtraTeacherCache {
		constructor() {
			this.teacherMap = /* @__PURE__ */ new Map();
		}
	...
	function createExcelLesNamesMap(excelLesSet) {
		let excelLesNamesMap = /* @__PURE__ */ new Map();
	...
	function perfectMatch(ctx, dko3Les, excelLesSet) {
		for (let excelLes of excelLesSet) {
	...
	function matchWithoutGradeYears(ctx, dko3Les, excelLesSet) {
		for (let excelLes of excelLesSet) {
			let weight = weigh1000(dko3Les, excelLes, void 0);
	...
	function matchWithoutLocation(ctx, dko3Les, excelLesSet) {
		for (let excelLes of excelLesSet) {
			let weight = weigh1000(dko3Les, excelLes, void 0);
			if (weight.diffSubject) continue;
			if (weight.diffDayTime) continue;
	...
	function matchWithoutTimeAndDay(ctx, dko3Les, excelLesSet) {
		for (let excelLes of excelLesSet) {
			let weight = weigh1000(dko3Les, excelLes, void 0);
			if (weight.diffSubject) continue;
			if (weight.diffLocation) continue;
	...
	function matchWithoutTeacherTimeAndDay(ctx, dko3Les, excelLesSet) {
		for (let excelLes of excelLesSet) {
			let weight = weigh1000(dko3Les, excelLes, void 0);
			if (weight.diffSubject) continue;
			if (weight.diffLocation) continue;
	...
	function matchWithoutTeacher(ctx, dko3Les, excelLesSet) {
		for (let excelLes of excelLesSet) {
			let weight = weigh1000(dko3Les, excelLes, void 0);
			if (weight.diffSubject) continue;
			if (weight.diffDayTime) continue;
	...
	function getDiffsCloudFileName(academie, schoolYear) {
		return `Dko3/${academie}_${schoolYear}_diffs.json`;
	}
	async function getDiffsFromCloud(academie, schoolYear) {
		let fromCloud = await cloud.json.fetch(getDiffsCloudFileName(academie, schoolYear)).catch(function(err) {
			console.log(err);
			return null;
		});
		if (fromCloud) {
			...
			for (let orphan of fromCloud.orphanedExcelLessen) if (!orphan.gradeYears) orphan.gradeYears = [];
		}
		return fromCloud;
	}
	...
	async function getUrlForWorksheet(workBook, workSheet, cellAddress, academie, schoolYear) {
		let url = (await getJsonDiffsCached(academie, schoolYear)).workBooks.find((wb) => wb.name == workBook)?.worksheets.find((ws) => ws.name == workSheet)?.url;
	...
	async function calcAndShowDiffsWww() {
		let text = await (await fetch("https://academieberchem.stedelijkonderwijs.be/uurrooster-woord-gevorderden-18")).text();
		console.log(text);
	}
	async function setupDiffPage() {
		...
		emmet.insertAfter(divInfo, "div#diffErrors.errors").last;
	...
	function getDiffsLocalFirst(academie, schoolYear) {
		let fileName = getDiffsCloudFileName(academie, schoolYear);
		let jsonDiff = sessionStorage.getItem(fileName);
		if (jsonDiff) return JSON.parse(jsonDiff);
		return getDiffsFromCloud(academie, schoolYear);
	}
	async function addDiff(titleHeader, academie, schoolYear) {
	...
	async function createSnapshot(schoolYear, reportStatus) {
		...
		await uploadSnapshotData({
			lessen: (await scrapeAllNormalLessen(schoolYear, reportStatus)).map((l) => l.les).map((les) => {
				return {
					id: les.id,
