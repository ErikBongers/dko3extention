(() => {
  ...
  var STORAGE_PAGE_SETTINGS_KEY_PREFIX = "pageSettings_";
  ...
  var globalTransientPageState = /* @__PURE__ */ new Map();
...
${yrNow}-${yrNext}`, classList: ["editable_number"], factor: 1, getValue: (ctx) => parseInt(ctx.data.fromCloud.columnMap.get(`uren_${yrNow}_${yrNext}`)?.get(ctx.vakLeraar.id)), totals: true });
  ...
  function onMutation4(_mutation) {
    ...
    if (!findTableRefInCode()?.navigationData.isOnePage()) {
      addTableNavigationButton(navigationBars, DOWNLOAD_TABLE_BTN_ID, "download full table", () => {
  ...
  function onButtonBarChanged() {
    ...
    addButton(targetButton, COUNT_BUTTON_ID, "Toon telling", onShowLerarenUren, "fa-guitar", ["btn-outline-info"]);
  ...
  function onMutation7(_mutation) {
