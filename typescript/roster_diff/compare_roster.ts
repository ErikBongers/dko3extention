...
function scrapeUurrooster(data: ExcelData) {
    daysRow = findDaysRow(data);
    if(daysRow === undefined)  {
        setError( "Geen rij met dagnamen gevonden.");
        return;
    }
    periodColumn = findPeriodColumn(data);
    let lastPeriodRow = findLastPeriodRow();
    data = data.data.slice(0, lastPeriodRow+1);
    if(periodColumn === undefined)  {
        setError( "Geen kolom met lesmomenten gevonden.");
        return;
    }
...
function _setMessage(msg: string, type: MessageType) {
    console.log(msg);
}
...
function setError(msg: string) {
    _setMessage(msg, MessageType.Error);
}

function setInfo(msg: string) {
    _setMessage(msg, MessageType.Info);
}

function setHighlight(msg: string) {
    _setMessage(msg, MessageType.Highlight);
}
